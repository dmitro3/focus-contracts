...
async function deploy() {
  ...
  let factory = await ethers.getContractFactory("FocusNFT1155", minter)
...
describe("FocusNFT1155", function () {
  it("Should deploy", async function() {
    const signers = await ethers.getSigners();
    const minter = signers[0].address;

    const FocusNFT1155 = await ethers.getContractFactory("FocusNFT1155");
    const focusNFT1155 = await FocusNFT1155.deploy(minter);
    await focusNFT1155.deployed();
  });
  ...
  it("Should redeem an NFT from a signed voucher", async function() {
    ...
    //buyer
    await expect(redeemerContract.redeem(redeemer.address, voucher, 1,1,[]))
      .to.emit(contract, 'TransferSingle')  // transfer from null address to minter
      .withArgs(redeemer.address,'0x0000000000000000000000000000000000000000', minter.address, voucher.tokenId,1)
      .and.to.emit(contract, 'TransferSingle') // transfer from minter to redeemer
      .withArgs( redeemer.address, minter.address, redeemer.address, voucher.tokenId,1);
  });

  it("Should fail to redeem an NFT more than supply", async function() {
    const { contract, redeemerContract, redeemer, minter } = await deploy()
  ...
  it("Should make payments available to minter for withdrawal", async function () {
    ...
    await expect(await redeemerContract.redeem(redeemer.address, voucher, 1, 1, [], { value: minPrice }))
